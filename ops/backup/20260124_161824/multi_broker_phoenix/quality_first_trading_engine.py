...
class QualityFirstTradingEngine:
    ...
    def __init__(self, min_quality_score: float = 70.0):
        ...
        self.order_size_defaults = {
            ...
            'default': float(os.getenv('AUTONOMOUS_DEFAULT_ORDER_USD', '500.0'))
        }
    ...
    def evaluate_and_execute_setup(self, setup: Dict[str, Any], 
        ...
        # Execute trade
        try:
            entry_price = setup.get('entry_price') or setup.get('current_price') or setup.get('fill_price') or 0
            size = setup.get('size', 0)
            broker_name = setup.get('broker', 'coinbase')

            if not size or size <= 0:
                size = self._determine_order_size(broker_name, entry_price)
                logger.info(f"   ℹ️  Using default notional ${size:.2f} for {broker_name}")

            if entry_price <= 0:
                entry_price = size if size > 0 else 1.0
    ...
    def _determine_order_size(self, broker_name: str, entry_price: float) -> float:
        """Pick a default USD notional when setups are missing a size."""
        target = self.order_size_defaults.get(broker_name)
        if target is None or target <= 0:
            target = self.order_size_defaults.get('default', 500.0)
        return target
