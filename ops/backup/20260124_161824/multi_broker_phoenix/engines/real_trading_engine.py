...
# === RBOTZILLA_FAIL_CLOSED_HIVE_WIRE (auto) ===
# Policy: if real AI hive voting is requested (XAI/DeepSeek), do NOT silently fall back to "simple hive".
# Fail closed so the system never runs with fake 0% votes again.
import os as _os

def _rbot_fail_closed(msg: str):
    raise RuntimeError("RBOTZILLA HIVE FAIL-CLOSED: " + msg)

def _rbot_require_real_hive():
    req_xai = _os.getenv("REQUIRE_XAI", "1").lower() in ("1","true","yes")
    req_ds  = _os.getenv("REQUIRE_DEEPSEEK", "1").lower() in ("1","true","yes")
    xai_ok  = bool(_os.getenv("XAI_API_KEY","").strip())
    ds_ok   = bool(_os.getenv("DEEPSEEK_API_KEY","").strip())
    if req_xai and not xai_ok:
        _rbot_fail_closed("REQUIRE_XAI=1 but XAI_API_KEY is empty")
    if req_ds and not ds_ok:
        _rbot_fail_closed("REQUIRE_DEEPSEEK=1 but DEEPSEEK_API_KEY is empty")
    return (req_xai or req_ds)
# === /RBOTZILLA_FAIL_CLOSED_HIVE_WIRE ===
...
class RealTradingEngine:
    ...
    def __init__(self, 
        ...
        # Initialize HIVE policy
        # By default, we DO NOT silently fall back to the simple (simulated) HIVE in live/real-money contexts.
        # Use the ALLOW_PAPER_WITHOUT_HIVE env var to explicitly permit the WSL-compatible simple hive when running in PAPER mode
        allow_paper_without_hive = _os.getenv("ALLOW_PAPER_WITHOUT_HIVE", "0").lower() in ("1","true","yes")
        trading_mode = _os.getenv('TRADING_MODE', 'PAPER').upper()

        # In LIVE mode, never allow simple fallback (fail-closed if real seats are not available)
        if trading_mode == 'LIVE':
            self.use_simple_hive = False
            # Validate that required external hive seats are available (throws on failure)
            try:
                _rbot_require_real_hive()
            except RuntimeError as e:
                raise RuntimeError(f"HIVE FAIL-CLOSED DURING ENGINE INIT: {e}")
        else:
            # PAPER mode: permit explicit allowance for simple hive when requested
            self.use_simple_hive = bool(allow_paper_without_hive)
        ...
        if not self.use_simple_hive:
            ...
            self.hive = None
            print("🐝 USING SIMPLE WSL-COMPATIBLE HIVE (EXPLICITLY ENABLED)")
    ...
    def evaluate_and_execute(self, candidate: TradeCandidate, market_data: Dict) -> Optional[Dict]:
        ...
        if self.use_simple_hive:
            ...
            try:
                ...
                print(f"   ⚠️ real_ai_hive import failed: {e}")
                get_real_ai_vote = None

# RBOTZILLA_PATCH_BEGIN: REQUIRE_XAI_DEEPSEEK_NO_FALLBACK
            # Fail-closed: ONLY allow XAI (Grok) and/or DeepSeek seats.
            # No OpenAI. No browser. No simulation fallback.
            use_xai = bool(os.getenv("XAI_API_KEY", "").strip())
            use_deepseek = bool(os.getenv("DEEPSEEK_API_KEY", "").strip())

            # Optional hard-disables (env-only; safe if unused elsewhere)
            disable_openai = os.getenv("DISABLE_OPENAI", "true").lower() in ("1","true","yes")
            disable_browser = os.getenv("DISABLE_BROWSER_HIVE", "true").lower() in ("1","true","yes")

            if not (use_xai or use_deepseek):
                print("❌ HIVE FAIL-CLOSED: No permitted AI seats available (need XAI_API_KEY and/or DEEPSEEK_API_KEY)")
                raise RuntimeError("HIVE_FAIL_CLOSED_NO_SEATS")

            if disable_openai:
                os.environ["OPENAI_API_KEY"] = ""  # force off, even if present elsewhere
            if disable_browser:
                os.environ["HIVE_BROWSER_ENABLED"] = "false"
# RBOTZILLA_PATCH_END: REQUIRE_XAI_DEEPSEEK_NO_FALLBACK
                    ...
                    try:
                        ...
                        if 'votes' in hive_result:
                            ...
                            for vote in hive_result['votes']:
                                print(f"       {vote.get('ai','ai')} ({vote.get('agent','agent')}): {vote['signal']} ({vote['confidence']:.1%})")

                        approved = hive_result["vote"] == "approve"
                        approval_rate = hive_result.get("consensus", hive_result.get("confidence", 0.0))

                    except Exception as e:
                        print(f"   ❌ REAL AI HIVE call failed: {e}")
                        print(f"   💡 Falling back to local simple HIVE")
                        ...
                        print(f"   🧾 Fallback HIVE vote: {hive_result['vote']} ({hive_result.get('confidence',0):.1%})")
                        approved = hive_result["vote"] == "approve"
                        approval_rate = hive_result.get("consensus", hive_result.get("confidence", 0.0))
                else:
