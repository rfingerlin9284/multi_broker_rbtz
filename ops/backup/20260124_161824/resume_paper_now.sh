...
# - NEVER unlocks guard (prints command only)
...
# 4) guard status
say "========================================="
say " Step 4: Guard Status"
say "========================================="
GUARD_LOCKED=0
GUARD_UNLOCK_CMD=""
if [[ -x "tools/guardctl.sh" ]]; then
  output=$(bash tools/guardctl.sh status 2>&1 || true)
  echo "$output"
  
  if echo "$output" | grep -qi "LOCKED"; then
    GUARD_LOCKED=1
    say ""
    say "⚠️  Guard is LOCKED - new entries frozen"
    say "   (Exits and position management will still work)"
    
    # Try to extract unlock command
    if bash tools/guardctl.sh help 2>/dev/null | grep -qi "unlock"; then
      GUARD_UNLOCK_CMD="bash tools/guardctl.sh unlock [PIN]"
    else
      GUARD_UNLOCK_CMD="bash tools/guardctl.sh unlock"
    fi
  fi
else
  say "⚠️  tools/guardctl.sh not found (skipping)"
fi
say ""

# 5) locate and stop old engine
say "========================================="
...
# 6) start engine under deterministic env
say "========================================="
say " Step 6: Start Engine (PAPER mode)"
say "========================================="
...
# Set PYTHONPATH to nested MULTI_BROKER_PHOENIX directory where modules exist
...
# 7) verify engine is alive
say "========================================="
say " Step 7: Verify Engine Process"
say "========================================="
...
# 8) proof-of-life: verify critical ticks
say "========================================="
say " Step 8: Proof-of-Life Verification"
say "========================================="
...
# 9) show recent log context
say "========================================="
say " Step 9: Recent Log Context"
say "========================================="
...
# 11) get env_state.json sha for report
...
report = {
  ...
  "guard_state": "locked" if $GUARD_LOCKED else "unlocked",
  "proof_of_life": {
...
if [[ "$verdict" == "PASS" ]]; then
  if [[ $GUARD_LOCKED -eq 1 ]]; then
    ...
    say ""
    say "⚠️  Guard is LOCKED - new entries are frozen"
    say ""
    say "Current state:"
    say "  ✅ Exits will work (positions can be closed)"
    say "  ✅ Stop losses will trigger"
    say "  ✅ Max hold time enforced (6-8 hours)"
    say "  ❌ New trade entries BLOCKED by guard"
    say ""
    if [[ -n "$GUARD_UNLOCK_CMD" ]]; then
      say "IF YOU WANT NEW TRADES, run:"
      say "  $GUARD_UNLOCK_CMD"
    else
      say "To unlock guard, see: bash tools/guardctl.sh help"
    fi
  else
    say "✅ Engine is healthy and operational"
    say "✅ Guard is unlocked - trading enabled"
    say ""
