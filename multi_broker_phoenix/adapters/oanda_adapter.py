...
class OandaAdapter:
    ...
    def place_oco(self, oco: OCOOrder) -> dict[str, Any]:
        ...
        if oco.quantity is None or oco.quantity <= 0:
            raise ValueError(
                f"OCO requires positive quantity. broker=oanda symbol={oco.symbol} qty={oco.quantity}"
            )
        
        # Determine units (OANDA uses signed units: +100 = long, -100 = short)
        # Handle both "BUY"/"SELL" and "LONG"/"SHORT" formats
        side_lower = oco.side.lower()
        is_long = side_lower in ('buy', 'long')
        units = oco.quantity if is_long else -oco.quantity
        ...
        try:
            ...
            order_cancel = response.get('orderCancelTransaction', {}) or response.get('orderRejectTransaction', {})
            cancel_reason = order_cancel.get('reason') if isinstance(order_cancel, dict) else None
            ...
            entry_status = 'filled' if order_fill else 'submitted'
            if order_cancel:
            ...
            sl_order = OrderInfo(
                ...
                order_type='STOP',
                price=oco.stop_loss,
                status='pending' if not order_cancel else 'canceled',
                timestamp=timestamp,
                error=cancel_reason,
            )
            ...
            if order_cancel:
                logger.warning(
                    f"❌ OANDA OCO canceled: {oco.symbol} {oco.side} "
                    f"qty={oco.quantity} reason={cancel_reason} "
            ...
            return {
                ...
                "placement_failed": bool(order_cancel),
