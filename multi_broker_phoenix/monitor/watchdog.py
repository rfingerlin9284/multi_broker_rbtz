...
def run_watchdog_once(oanda_connector, grok_url: Optional[str], deepseek_url: Optional[str], invoke_callbacks_async: bool = True) -> Dict[str, Any]:
    ...
    if os.getenv('WATCHDOG_AI_HEALTH', '1').lower() in ('1', 'true', 'yes'):
        try:
            ...
            require_ai = os.getenv('WATCHDOG_REQUIRE_AI_SEATS', '0').lower() in ('1', 'true', 'yes')
            details['ai_seats'] = {
                'ok_any': ok_any,
                'chosen': chosen,
                'snapshot': snap,
    ...
    # Write watchdog heartbeat first so the self-check does not stale itself.
    ...
    if not ok:
        ...
        try:
            print("WATCHDOG_OK trading_allowed=true", flush=True)
        except Exception:
            pass
