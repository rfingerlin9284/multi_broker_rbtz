...
# Usage: ./start_full_system.sh [--unlock]
# Default: starts with GUARD LOCKED (safe mode, no trading)
# With --unlock: starts with trading enabled (use with caution)
...
GUARD_LOCKED="${GUARD_DEFAULT_LOCKED:-1}"
...
export GUARD_DEFAULT_LOCKED="$GUARD_LOCKED"
...
if [[ "${1:-}" == "--unlock" ]]; then
  GUARD_LOCKED=0
  UNLOCK_ARG="--unlock"
  echo -e "${YELLOW}⚠${NC} UNLOCK requested. Guard will be UNLOCKED (trading enabled if brokers healthy)."
fi
...
echo -e "  Guard:     $([ "$GUARD_LOCKED" == "1" ] && echo -e "${GREEN}LOCKED (SAFE)${NC}" || echo -e "${RED}UNLOCKED${NC}")"
echo ""
...
if [[ "${BROKER_OANDA_ENABLED:-0}" == "1" || "${OANDA_ENABLED:-0}" == "1" ]]; then
  ...
  echo "═══════════════════════════════════════════════════════════════"
  ./tools/preflight_oanda.sh || { echo ""; echo "Preflight failed. Trading remains locked."; exit 1; }
fi

echo -e "${GREEN}═══════════════════════════════════════════════════════════════${NC}"
echo ""
echo -e "Run ${BLUE}./tools/status_full_system.sh${NC} to check broker health"
echo -e "Run ${BLUE}./tools/guardctl.sh unlock${NC} to enable trading (if guard locked)"
echo ""
