...
# Import Progressive Position Manager for trailing stops
try:
    from multi_broker_phoenix.engines.progressive_position_manager import (
        ProgressivePositionManager, ProgressiveStage, PositionSide, ManagedPosition
    )
    TRAILING_STOP_AVAILABLE = True
except ImportError:
    TRAILING_STOP_AVAILABLE = False
    print("⚠️  Progressive Position Manager not available - trailing stops disabled")
...
def run_headless(mode: Optional[str] = None):
    ...
    # Initialize Progressive Position Manager for trailing stops
    position_manager = None
    trailing_enabled = os.getenv('TRAILING_STOP_ENABLED', 'true').lower() in ('true', '1', 'yes')
    if TRAILING_STOP_AVAILABLE and trailing_enabled:
        # Configure stages from env or use defaults
        # Format: profit_threshold:close_pct:trail_pips (comma-separated)
        stages_config = os.getenv('TRAILING_STAGES', '30:0.25:12,60:0.25:8,100:0.25:5')
        stages = []
        for stage_str in stages_config.split(','):
            parts = stage_str.strip().split(':')
            if len(parts) == 3:
                try:
                    stages.append(ProgressiveStage(
                        profit_threshold=float(parts[0]),
                        close_percentage=float(parts[1]),
                        trailing_stop_pips=float(parts[2])
                    ))
                except ValueError:
                    pass
        ...
        if not stages:
            ...
            stages = [
                ...
                ProgressiveStage(profit_threshold=60, close_percentage=0.25, trailing_stop_pips=8),
                ProgressiveStage(profit_threshold=100, close_percentage=0.25, trailing_stop_pips=5),
            ]
        
        initial_stop = float(os.getenv('TRAILING_INITIAL_PIPS', '20'))
        position_manager = ProgressivePositionManager(stages=stages, initial_stop_pips=initial_stop)
        print(f"✅ Progressive Position Manager: ACTIVATED")
        print(f"   🎯 Trailing stages: {len(stages)} configured")
        print(f"   📊 Initial stop: {initial_stop} pips")
        if canary_mode:
            _canary_log(f"Trailing stops ACTIVATED with {len(stages)} stages")
    else:
        if not TRAILING_STOP_AVAILABLE:
            print("⚠️  Progressive Position Manager not available")
        else:
            print("⚠️  Trailing stops DISABLED (set TRAILING_STOP_ENABLED=true to enable)")
    ...
    try:
        ...
        while True:
            ...
            for sym in symbols:
                ...
                # ============================================================
                # TRAILING STOP MANAGEMENT: Update open positions
                # ============================================================
                if position_manager is not None and sym in position_manager.positions:
                    try:
                        actions = position_manager.update_position(sym, price)
                        for action in actions:
                            action_type = action.get('type')
                            
                            if action_type == 'PARTIAL_CLOSE':
                                # Execute partial close via broker
                                close_size = action.get('size', 0)
                                reason = action.get('reason', 'Stage trigger')
                                profit = action.get('profit', 0)
                                print(f"🎯 TRAILING: Partial close {sym} - {close_size:.0f} units ({reason})")
                                print(f"   💰 Profit locked: ${profit:.2f}")
                                
                                # Route to appropriate broker for partial close
                                if oanda and '_' in sym:
                                    try:
                                        # OANDA partial close
                                        oanda.close_partial_position(sym, int(close_size))
                                    except Exception as e:
                                        print(f"   ⚠️  OANDA partial close failed: {e}")
                                elif coin and '-' in sym:
                                    try:
                                        coin.close_partial_position(sym, close_size)
                                    except Exception as e:
                                        print(f"   ⚠️  Coinbase partial close failed: {e}")
                                
                            elif action_type == 'UPDATE_STOP':
                                new_stop = action.get('new_stop', 0)
                                old_stop = action.get('old_stop', 0)
                                print(f"📈 TRAILING: {sym} stop updated ${old_stop:.5f} → ${new_stop:.5f}")
                                
                                # Update broker stop (if broker supports it)
                                if oanda and '_' in sym:
                                    try:
                                        oanda.update_stop_loss(sym, new_stop)
                                    except Exception as e:
                                        print(f"   ⚠️  OANDA stop update failed: {e}")
                                
                            elif action_type == 'STOP_HIT':
                                remaining = action.get('remaining_size', 0)
                                total_profit = action.get('total_profit', 0)
                                print(f"🛑 TRAILING: {sym} STOP HIT - Closing remaining {remaining:.0f} units")
                                print(f"   📊 Final P/L: ${total_profit:.2f}")
                                
                                # Close full position via broker
                                if oanda and '_' in sym:
                                    try:
                                        oanda.close_position(sym)
                                    except Exception as e:
                                        print(f"   ⚠️  OANDA close failed: {e}")
                                elif coin and '-' in sym:
                                    try:
                                        coin.close_position(sym)
                                    except Exception as e:
                                        print(f"   ⚠️  Coinbase close failed: {e}")
                                        
                    except Exception as pm_exc:
                        print(f"⚠️  Position manager update failed for {sym}: {pm_exc}")
                ...
                for strat_name, strategy in strategies.items():
                    ...
                    if position_manager is not None and order and order.get('status') != 'SKIPPED':
                        try:
                            order_id = order.get('order_id') or order.get('id') or f"{sym}_{datetime.now().timestamp()}"
                            side_enum = PositionSide.LONG if cand.side.upper() == 'BUY' else PositionSide.SHORT
                            position_manager.open_position(
                                symbol=sym,
                                side=side_enum,
                                entry_price=cand.entry_price or price,
                                size=sizing['size'],
                                broker_order_id=str(order_id)
                            )
                            print(f"📍 Position tracked for trailing stops: {sym}")
                            if canary_mode:
                                _canary_log(f"📍 Position tracked: {sym} {cand.side} @ {cand.entry_price or price:.5f}")
                        except Exception as pm_exc:
                            print(f"⚠️  Position tracking failed: {pm_exc}")
                try:
