...
class EMAScalper(Strategy):
    ...
    def generate_candidate(self, market_data: Dict[str, Any]) -> Optional[TradeCandidate]:
        ...
        if short > long and prices[-1] > prices[-2]:
            ...
            take_profit = entry * (1 + self.stop_loss_pct * 2)  # 2:1 R:R
            confidence = min(95.0, 60 + abs(short - long) / (abs(long) + 1e-12) * 100)
            cand = TradeCandidate('ema_scalper', symbol, platform, entry, stop, side='BUY', take_profit=take_profit)
        ...
        if short < long and prices[-1] < prices[-2]:
            ...
            take_profit = entry * (1 - self.stop_loss_pct * 2)  # 2:1 R:R
            confidence = min(95.0, 60 + abs(short - long) / (abs(long) + 1e-12) * 100)
            cand = TradeCandidate('ema_scalper', symbol, platform, entry, stop, side='SELL', take_profit=take_profit)
            setattr(cand, 'confidence', confidence)
            return cand
        return None
...
class InstitutionalSD(Strategy):
    ...
    def generate_candidate(self, market_data: Dict[str, Any]) -> Optional[TradeCandidate]:
        ...
        if last_ret > 0:
            ...
            cand = TradeCandidate('institutional_sd', symbol, platform, entry, stop, side='BUY', take_profit=take_profit)
            setattr(cand, 'confidence', confidence)
            return cand
        elif last_ret < 0:
            stop = entry * (1 + self.stop_loss_pct)
            take_profit = entry * (1 - self.stop_loss_pct * 2)  # 2:1 R:R
            confidence = min(95.0, 65 + (vol_ratio - 1) * 20)
            cand = TradeCandidate('institutional_sd', symbol, platform, entry, stop, side='SELL', take_profit=take_profit)
            setattr(cand, 'confidence', confidence)
            return cand
        return None
...
class TrapReversal(Strategy):
    ...
    def generate_candidate(self, market_data: Dict[str, Any]) -> Optional[TradeCandidate]:
        ...
        if abs((p1 - p2) / p2) > 0.02 and (p0 - p1) * (p1 - p2) < 0:
            ...
            risk = abs(entry - stop)
            take_profit = entry + (risk * 2) if side == 'BUY' else entry - (risk * 2)
            return TradeCandidate('trap_reversal', symbol, platform, entry, stop, side=side, take_profit=take_profit)
        return None
...
class HolyGrail(Strategy):
    ...
    def generate_candidate(self, market_data: Dict[str, Any]) -> Optional[TradeCandidate]:
        ...
        if is_uptrend and momentum > momentum_threshold:
            ...
            cand = TradeCandidate('holy_grail', symbol, platform, entry, stop, side='BUY', take_profit=take_profit)
            setattr(cand, 'confidence', confidence)
            return cand
        ...
        if is_downtrend and momentum < -momentum_threshold:
            ...
            take_profit = entry * (1 - self.stop_loss_pct * 2)  # 2:1 R:R
            confidence = min(95.0, 70 + abs(momentum) * 100)
            cand = TradeCandidate('holy_grail', symbol, platform, entry, stop, side='SELL', take_profit=take_profit)
            setattr(cand, 'confidence', confidence)
            return cand
