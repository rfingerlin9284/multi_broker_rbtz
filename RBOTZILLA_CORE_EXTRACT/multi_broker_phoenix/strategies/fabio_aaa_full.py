...
class FabioAAAFull(Strategy):
    ...
    def _build_tranche_plan(self, entry_price: float, side: str, stop_pct: float, instrument: str = 'EUR_USD') -> List[Dict[str, Any]]:
        ...
        For FOREX: Uses pip-based targets (2:1 R:R on first tranche)
        For CRYPTO: Uses percentage-based targets
        """
        import os
        ...
        risk_distance = abs(entry_price - base_stop)
        
        # Detect if this is FOREX or CRYPTO
        is_forex = '_' in instrument  # EUR_USD, GBP_USD etc have underscore
        
        # Take profit multipliers (R:R based for forex, % based for crypto)
        if is_forex:
            # FOREX: Use 2:1, 3:1, 4:1 R:R targets (reasonable pip targets)
            tp_multipliers = [2.0, 3.0, 4.0]  # Risk:Reward ratios
            for alloc, rr in zip(allocations, tp_multipliers):
                if dir_up:
                    take_price = entry_price + (risk_distance * rr)
                else:
                    take_price = entry_price - (risk_distance * rr)
            ...
            # CRYPTO: Use percentage multipliers (original behavior)
            takes = [1.02, 1.04, 1.06]  # 2%, 4%, 6% targets (more realistic)
    ...
    def generate_candidate(self, market_data: Dict[str, Any]) -> Optional[TradeCandidate]:
        ...
        # Build tranche plan (pass instrument for forex vs crypto detection)
        tranche_plan = self._build_tranche_plan(entry_price, side, stop_pct, instrument)

        # Primary stop for TradeCandidate is the base stop (the most conservative)
        primary_stop = tranche_plan[0]['stop_price']
        # Primary take profit is the first tranche's take price (2:1 R:R for forex)
        primary_take = tranche_plan[0]['take_price']

        cand = TradeCandidate('fabio_aaa_full', instrument, platform, entry_price, primary_stop, side=side, take_profit=primary_take)
