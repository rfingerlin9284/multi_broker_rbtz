...
class RiskManager:
    ...
    def size_for_trade(self, candidate, account_equity: float, base_risk_pct: Optional[float] = None, fee_pct: float = 0.0, slippage_pct: float = 0.0) -> Dict[str, float]:
        ...
        conf_boost_enabled = os.environ.get('CONFIDENCE_BOOST_ENABLED', 'false').lower() in ('true', '1', 'yes')
        if conf_boost_enabled:
            ultra_high_thresh = float(os.environ.get('ULTRA_HIGH_CONFIDENCE_THRESHOLD', '0.90'))
            high_thresh = float(os.environ.get('HIGH_CONFIDENCE_THRESHOLD', '0.80'))
            ultra_mult = float(os.environ.get('ULTRA_HIGH_CONFIDENCE_SIZE_MULT', '1.5'))
            high_mult = float(os.environ.get('HIGH_CONFIDENCE_SIZE_MULT', '1.3'))
            
            if confidence >= ultra_high_thresh:
                mult *= ultra_mult
                logger.info(f"🔥 ULTRA-HIGH confidence {confidence:.2%} → {ultra_mult}x size boost")
            elif confidence >= high_thresh:
                mult *= high_mult
                logger.info(f"⭐ HIGH confidence {confidence:.2%} → {high_mult}x size boost")
        
        # adjust for fees/slippage conservatively, but ensure minimum 0.25% effective risk for paper trading
        min_effective_risk = float(os.environ.get('MIN_EFFECTIVE_RISK_PCT', '0.0025'))  # 0.25% minimum
        effective_pct = max(min_effective_risk, effective_pct - (fee_pct + slippage_pct))
        ...
        # Enforce notional bounds (min and max)
        entry_price = float(getattr(candidate, 'entry_price', 0.0))
        min_notional = float(os.environ.get('MIN_NOTIONAL_USD', '5000'))
        max_notional = float(os.environ.get('MAX_NOTIONAL_USD', '999999'))
        if entry_price > 0:
            notional = size * entry_price
            if notional < min_notional * 0.8:  # Below floor
                size = (min_notional / entry_price)
            elif notional > max_notional:  # Above cap
    ...
    def can_place_trade(self, proposed_risk_pct: float, open_trades_count: int, total_open_risk_pct: float) -> Dict[str,Any]:
        max_concurrent = int(os.environ.get('MAX_CONCURRENT_TRADES', self.config.get('max_concurrent_trades', 4)))
        max_risk_pct = float(os.environ.get('MAX_TOTAL_OPEN_RISK_PCT', self.config.get('max_total_open_risk_pct', 0.05)))
        ...
        if total_open_risk_pct + float(proposed_risk_pct) > max_risk_pct:
