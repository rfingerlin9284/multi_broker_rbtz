...
def run_headless(mode: Optional[str] = None):
    ...
    try:
        ...
        if canary_mode:
            _canary_log("Coinbase connector initialized")
    except Exception as exc:
    ...
    try:
        ...
        if canary_mode:
            _canary_log("IBKR connector initialized")
    except Exception as exc:
        print(f"⚠️  IBKR connector failed to initialize; continuing: {exc}")
        if canary_mode:
    ...
    if OANDAConnector is not None and os.getenv('OANDA_API_TOKEN'):
        ...
        if practice_account and (practice_flag or oanda_mode in ('PAPER','PRACTICE') or os.getenv('TRADING_MODE','PAPER').upper() != 'LIVE'):
            ...
            print(f"⚙️ Using OANDA practice account: {oanda_account}")
        else:
            print(f"⚙️ Using OANDA account: {oanda_account}")

        oanda = OANDAConnector(token=os.getenv('OANDA_API_TOKEN'), account_id=oanda_account, base_url=os.getenv('OANDA_API_URL', 'https://api-fxpractice.oanda.com'), engine=engine)
