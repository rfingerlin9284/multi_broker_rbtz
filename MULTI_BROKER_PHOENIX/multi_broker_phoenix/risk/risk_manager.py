...
class RiskManager:
    ...
    def size_for_trade(self, candidate, account_equity: float, base_risk_pct: Optional[float] = None, fee_pct: float = 0.0, slippage_pct: float = 0.0) -> Dict[str, float]:
        ...
        # Enforce minimum notional value (~10k ±20% = 8k-12k target)
        entry_price = float(getattr(candidate, 'entry_price', 0.0))
        min_notional = float(os.environ.get('MIN_NOTIONAL_USD', '10000'))  # $10k default
        if entry_price > 0:
            notional = size * entry_price
            if notional < min_notional * 0.8:  # Below 8k floor
                # Scale up to minimum notional
