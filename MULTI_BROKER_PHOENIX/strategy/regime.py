...
Outputs regime: 'TREND'|'RANGE'|'CHAOP', confidence and reasons.
Uses ADX proxy (absolute price movement), ATR, and EMA slope.
"""
...
def classify(prices: list, cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Simple ADX proxy: normalized directional movement via range vs close
    diffs = [abs(prices[i] - prices[i-1]) for i in range(1, len(prices))]
    atr = sum(diffs) / len(diffs)
    # ADX proxy: mean absolute change over average volatility
