...
class OANDAConnector:
    ...
    def get_current_price(self, instrument: str) -> Optional[float]:
        ...
        return self.get_last_price(instrument)


    def get_prices(self, instruments: list[str]) -> Dict[str, Dict[str, float]]:
        """Return bid/ask for instruments.

        Output: {"EUR_USD": {"bid": 1.0, "ask": 1.1}, ...}
        """
        if not instruments:
            return {}
        ...
        inst = ','.join(sorted(set(instruments)))
        try:
            resp = self.http.get(
    ...
    def get_positions(self) -> list[Dict[str, Any]]:
        """Return open trades as position-like dicts."""
    ...
    def get_historical_data(self, instrument: str, count: int = 100, granularity: str = 'M5') -> list[Dict[str, Any]]:
        """Fetch historical candle data for a symbol (midpoint)."""
        if not self.token or not self.account_id:
            return []
        try:
            resp = self.http.get(
            ...
            logger.warning('get_historical_data failed: %s', exc)
            return []
