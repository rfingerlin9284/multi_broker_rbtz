...
class OandaPracticeClient:
    ...
    def close_position(self, instrument: str) -> Dict[str, Any]:
        """Close position for the given instrument (close both sides: long and short)."""
        payload = {
            'longUnits': 'ALL',
            'shortUnits': 'ALL'
        }
        return self._put(f'/v3/accounts/{self.account_id}/positions/{instrument}/close', json=payload)
    ...
    def create_trailing_stop(self, instrument: str, trade_id: str, distance: float) -> Dict[str, Any]:
        ...
        return self._post(f'/v3/accounts/{self.account_id}/trades/{trade_id}/orders', json=payload)

    def create_stop_loss(self, trade_id: str, price: float) -> Dict[str, Any]:
        ...
        payload = {
            'tradeID': trade_id,
            'price': f"{float(price):.5f}"  # formatting; broker will validate precision
        }
        return self._post(f'/v3/accounts/{self.account_id}/trades/{trade_id}/orders', json=payload)

    def create_take_profit(self, trade_id: str, price: float) -> Dict[str, Any]:
