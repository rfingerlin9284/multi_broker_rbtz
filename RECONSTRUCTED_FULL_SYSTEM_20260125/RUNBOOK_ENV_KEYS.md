...
9. **NEVER unlocks guard** (prints instructions only)

**Verdict:**

- ✅ **PASS:** Both ticks detected → safe to unlock guard (if needed)
- ❌ **FAIL:** Missing ticks → **DO NOT unlock guard** → fix exits/protect loop first
...
- Will print unlock command (but NOT execute it)
- Shows: `bash tools/guardctl.sh unlock [PIN]`

**NEVER unlock guard until:**

1. `verdict=PASS`
2. Exit/protect ticks detected
3. No ERROR/Traceback spam in logs
...
Script will print **but NOT execute**:

```
⚠️  GUARD IS LOCKED - Entries are intentionally frozen

To unlock guard (ONLY after verifying above):
  bash tools/guardctl.sh unlock [PIN]
```
