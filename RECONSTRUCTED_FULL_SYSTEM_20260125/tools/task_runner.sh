...
if [[ -f "$ROOT/config/toggles.env" ]]; then
  toggles_hash="$(python3 - "$ROOT" <<'PY'
import hashlib
import sys
p = f"{sys.argv[1]}/config/toggles.env"
try:
    data = open(p, 'rb').read()
    print(hashlib.sha256(data).hexdigest())
except Exception:
...
summary_hash="$(python3 - "$ROOT" "$task_name" "$stamp" <<'PY'
import hashlib
import sys
root = sys.argv[1]
task = sys.argv[2]
stamp = sys.argv[3]
p = f"{root}/ops/task_runs/{task}/{stamp}.summary.txt"
try:
  data = open(p, 'rb').read()
  print(hashlib.sha256(data).hexdigest())
except Exception:
