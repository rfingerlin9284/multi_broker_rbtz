...
def _load_env_file():
    ...
    engine_root = script_dir.parent  # .../MULTI_BROKER_PHOENIX
    project_root = engine_root.parent  # .../RECONSTRUCTED_FULL_SYSTEM_20260125

    # Ensure both roots are importable
    for path in (str(project_root), str(engine_root)):
        if path not in sys.path:
            sys.path.insert(0, path)
    env_path = project_root / '.env'
    fallback_env_path = engine_root / '.env'

    if not env_path.exists() and fallback_env_path.exists():
